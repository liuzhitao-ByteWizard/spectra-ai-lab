...
export function buildExperimentReportHtml(record: SavedRecord) {
  ...
  const reportable = result.reportable === true;
  ...
  const title = `${record.source}已知谱线光栅常数测量报告`;
  const conclusion = reportable
    ? `本次测量得到${record.resultLabel}为 ${record.resultValue}。结果通过当前图像质量、参数边界与可辨识性检查，可作为本次实验的报告结果；仍需结合数据限制理解其适用范围。`
    : `本次记录尚未形成可报告的最终结果。当前状态为“${record.quality}”，应完成缺失步骤或按诊断意见复核后再提交实验结论。`;
